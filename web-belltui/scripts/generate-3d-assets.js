...
async function meshyCreatePreview(prompt, stylePrefix) {
  const fullPrompt = `${stylePrefix} ${prompt}`;
  await log(`[Meshy] Preview 생성 요청: ${fullPrompt.slice(0, 80)}...`);

  const data = await meshyRequest('/text-to-3d', {
    method: 'POST',
    body: JSON.stringify({
      mode: 'preview',
      prompt: fullPrompt,
...
async function meshyCreateRefine(previewTaskId) {
  await log(`[Meshy] Refine 생성 요청: preview=${previewTaskId}`);
...
async function meshyPollTask(taskId) {
  while (true) {
    const data = await meshyRequest(`/text-to-3d/${taskId}`);
...
async function generateWithMeshy(asset, stylePrefix) {
  // Step 1: Preview
  const previewId = await meshyCreatePreview(asset.prompt, stylePrefix);
  const previewResult = await meshyPollTask(previewId);

  // Step 2: Refine
  const refineId = await meshyCreateRefine(previewId);
  const refineResult = await meshyPollTask(refineId);

  // Step 3: Download
  const outputPath = path.join(CONFIG.rawDir, `${asset.id}.glb`);
  const fileSize = await meshyDownloadGlb(refineResult, outputPath);
